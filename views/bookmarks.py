...
class BookmarksListEndpoint(Resource):
    ...
    def post(self):
        ...
        book = Post.query.filter_by(id = curr).filter(Post.user_id.in_(users)).all()
...
class BookmarkDetailEndpoint(Resource):
    ...
    def delete(self, id):
        ...
        if not bookmark:
            return Response(json.dumps({'message': 'not a bookmark'}),  mimetype="application/json", status=401)

        # you should only be able to edit/delete posts that are yours
        if bookmark.user_id != self.current_user.id:
             return Response(json.dumps({'message': 'cannot delete this bookmark'}),  mimetype="application/json", status=401)

        Bookmark.query.filter_by(id=id).delete() 
        db.session.commit()
        return Response(json.dumps({'message':'bookmark deleted'}), mimetype="application/json", status=201)
